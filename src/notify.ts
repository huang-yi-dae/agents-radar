 ...
 *
 * LEARNING NOTES:
 * - This module runs AFTER the digest pipeline completes.
 * - It reads manifest.json to find the latest date and available reports.
 * - The Telegram Bot API is a simple REST API — POST to /sendMessage with HTML formatting.
 */
...
/** Highlights structure (same as in rollup.ts). */
 ...
 *
 * HOW IT WORKS:
 * - Lists each report with a link to the Web UI (hash routing).
 * - If highlights are available, adds them as indented sub-items.
 * - Detects weekly/monthly reports and adjusts the message header.
 *
 * @param date - Date string
 * @param reports - Array of report IDs available for this date
 * @param pagesUrl - Optional base URL override
 * @param highlights - Optional highlights for this date
...
// ── SUMMARY ──────────────────────────────────────────────────────────────────
// Telegram notification module. Key patterns:
// 1. Reads manifest.json to find the latest reports
// 2. Loads highlights.json for rich notifications
// 3. Uses Telegram Bot API (simple REST POST)
//
// QUESTIONS:
// Q1: Why read manifest.json instead of scanning digests/ directly?
//     (Answer: manifest.json is the canonical source of truth for what reports exist.
//      It's generated by generate-manifest.ts after all reports are saved.)
// ─────────────────────────────────────────────────────────────────────────────
