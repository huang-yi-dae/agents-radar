 ...
 * - prompts.ts: repo-level prompts (per-tool digests, comparisons)
 * - prompts-data.ts: data-source prompts (trending, web, HN) and rollups (weekly, monthly)
 *
 * LEARNING NOTES:
 * - Each function takes raw data from a specific source and builds a prompt string.
 * - The "three-step" pattern in trending/web prompts: filter → categorize → report.
 * - Rollup prompts take previously-generated daily/weekly digests as input.
 */
 ...
 * - Formats each repo as a Markdown list item with stars, language, description.
 * - Instructs the LLM to: (1) filter for AI relevance, (2) categorize by dimension,
 *   (3) generate a structured report with trend signals.
 *
 * @param data - TrendingData with trendingRepos and searchRepos arrays
...
/**
 * Build the prompt for the web content report (Anthropic + OpenAI).
 *
 * Handles two modes:
...
/**
 * Build the prompt for weekly rollup reports.
 * Takes the last 7 daily digests as input and asks the LLM to synthesize them.
 */
...
/**
 * Build the prompt for monthly rollup reports.
 * Takes weekly digests (or sampled daily digests) as input.
 */
...
/**
 * Interface for structured highlights — maps report IDs to arrays of highlight strings.
 * Example: { "ai-cli": ["Claude Code released v2.0", "Codex added streaming support"] }
 */
...
/**
 * Build the prompt to extract highlights from finished reports.
 *
 * HOW IT WORKS:
 * - Takes the content of each report (truncated to 2000 chars).
 * - Asks the LLM to return ONLY valid JSON with highlight strings.
 * - The JSON response is parsed in index.ts and used for Telegram notifications.
 *
 * SPECIAL: This prompt asks for JSON output, not Markdown.
 * The LLM is instructed to return raw JSON without markdown code fences.
 *
 * @param reportContents - Map of report ID to report content
 * @param lang - Language
 * @param itemsPerReport - Number of highlights per report (default: 6)
 * @returns The complete prompt string
 */
...
/**
 * Build the prompt for the Hacker News AI community digest.
 */
...
// ── SUMMARY ──────────────────────────────────────────────────────────────────
// This module builds prompts for data-source reports and rollups.
// Key difference from prompts.ts: these deal with unstructured data (web pages, HN stories,
// trending repos) rather than structured GitHub API data.
//
// Pattern: each function takes raw data → formats it as text → adds instructions → returns prompt.
//
// QUESTIONS:
// Q1: Why does buildHighlightsPrompt ask for JSON instead of Markdown?
//     (Answer: The highlights are machine-parsed for Telegram notifications,
//      not displayed as a report — JSON is easier to parse programmatically)
// Q2: What's the difference between buildWeeklyPrompt and buildMonthlyPrompt inputs?
//     (Answer: Weekly takes daily digests; monthly takes weekly digests OR sampled daily digests)
// ─────────────────────────────────────────────────────────────────────────────
