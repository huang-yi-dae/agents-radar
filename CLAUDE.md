...
export LLM_PROVIDER=anthropic   # anthropic | openai | github-copilot | openrouter | stepfun
...
- **OPENCLAW** + **OPENCLAW_PEERS** (12): openclaw/openclaw + 12 peer projects (sorted by stars)
...
- Daily runs write `digests/YYYY-MM-DD/highlights.json`; Telegram notifications read it via `src/notify.ts`.

## Web UI & RSS Feed

- **Web UI**: `index.html` reads `manifest.json` to build the sidebar, then fetches the selected dated report on demand.
- RSS Feed: `feed.xml` at the repo root. Generated by `src/generate-manifest.ts` in the same `pnpm manifest` step. Contains the latest 30 items (newest first) across all report types. Item links use hash routing: `https://huang-yi-dae.github.io/agents-radar/#YYYY-MM-DD/report`.
- GitHub Actions commits `manifest.json` and `feed.xml` together in the manifest step.
